...
class RoboMimicDataset(Dataset):
    ...
    def __init__(
        ...
        if train_filter_key is None:
            all_demos = list(self.hdf5_file["data"].keys())
            all_demos = [demo.encode("utf8") for demo in all_demos]
        else:
        ...
        if only_use_this_demos is not None:
            this_demos = [demo.encode("utf8") for demo in only_use_this_demos]
            self.all_demos = [demo for demo in self.all_demos if demo in this_demos]
    ...
    def __getitem__(self, index):
        ...
        if self.predict_delta_pos:
            if actions.shape[-1] == 10:
                ...
                raise NotImplementedError("predict_delta_pos only implemented for action_dim 10 or 20, got {}".format(actions.shape[-1]))
