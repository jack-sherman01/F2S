...
class MIDP(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.style_infuser = EncoderMLP(
            input_dim=readout_dim + self.style_dim,
            output_dim=readout_dim,
            hidden_dim=hidden_dim,
        )
    ...
    def forward(
        ...
        obs_embedding = self.bottleneck(obs_embedding)
        if style is None:
            # sample style
            style = torch.randn(obs_embedding.shape[0], self.style_dim, device=obs_embedding.device) * self.noise_scale
        readout = self.style_infuser(torch.cat([obs_embedding, style], dim=-1))
        ...
        if actions is not None:
            ...
            readout_origin = self.style_infuser(torch.cat([obs_embedding, style_origin], dim=-1))
