# modified from https://github.com/real-stanford/diffusion_policy/blob/main/diffusion_policy/scripts/robomimic_dataset_conversion.py
...
from robomimic_util import RobomimicAbsoluteActionConverter
...
def main(input, output, eval_dir, num_workers):
    convert_rel_actions_to_abs(input, output, eval_dir, num_workers)

def convert_rel_actions_to_abs(input, output, eval_dir=None, num_workers=None):
    # process inputs
    ...
    with h5py.File(output, 'r+') as out_file:
        for i in tqdm(range(len(converter)), desc="Writing to output"):
            ...
            if 'actions' in demo:
                del demo['actions']
            # add new actions
            demo.create_dataset('actions', data=abs_actions)
