# modified from https://github.com/real-stanford/diffusion_policy/blob/main/diffusion_policy/common/robomimic_util.py
...
class RobomimicAbsoluteActionConverter:
    ...
    def convert_actions(self, 
        ...
        for i in range(len(states)):
            ...
            for idx, robot in enumerate(env.env.robots):
                ...
                action_goal_ori[i,idx] = self.rotation_transformer.forward(
        ...
        abs_actions = stacked_abs_actions.reshape(*actions.shape[:-1], -1)
        return abs_actions
    ...
    def convert_and_eval_idx(self, idx):
        ...
        delta_error_info = self.evaluate_rollout_error(
            ...
            metric_skip_steps=eval_skip_steps, is_abs_action=False)
        abs_error_info = self.evaluate_rollout_error(
            abs_env, states, abs_actions, robot0_eef_pos, robot0_eef_quat,
            metric_skip_steps=eval_skip_steps, is_abs_action=True)
    ...
    def evaluate_rollout_error(
        ...
        for i in range(len(states)):
            ...
            action = actions[i]
            if is_abs_action:
                action = self.rotation_transformer.undo_transform_action(action)
