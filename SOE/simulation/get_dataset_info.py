...
if __name__ == "__main__":
    ...
    actions = np.concatenate([f["data/{}/actions".format(ep)][()] for ep in demos], axis=0)
    action_min_per_dim = np.min(actions, axis=0)
    action_max_per_dim = np.max(actions, axis=0)
    action_mean_per_dim = np.mean(actions, axis=0)
    action_std_per_dim = np.std(actions, axis=0)
    params_for_normalization = {
        "min_val": [round(x, 3) for x in action_min_per_dim.tolist()],
        "max_val": [round(x, 3) for x in action_max_per_dim.tolist()],
        "mean_val": [round(x, 3) for x in action_mean_per_dim.tolist()],
        "std_val": [round(x, 3) for x in action_std_per_dim.tolist()],
    }
    params_for_normalization = json.dumps(
        params_for_normalization, 
    )

    # action_min_per_dim = np.min(f["data/{}/actions".format(demos[0])][()], axis=0)
    # action_max_per_dim = np.max(f["data/{}/actions".format(demos[0])][()], axis=0)
    # action_mean_per_dim = np.mean(f["data/{}/actions".format(demos[0])][()], axis=0)
    # for ep in demos[1:]:
    #     action_min_per_dim = np.minimum(action_min_per_dim, np.min(f["data/{}/actions".format(ep)][()], axis=0))
    #     action_max_per_dim = np.maximum(action_max_per_dim, np.max(f["data/{}/actions".format(ep)][()], axis=0))
    #     action_mean_per_dim += np.mean(f["data/{}/actions".format(ep)][()], axis=0)
    # action_mean_per_dim /= len(demos)
    ...
    print("params for normalization: {}".format(params_for_normalization))
    print("")
    print("==== Filter Keys ====")
