...
class CameraD400(object):
    def __init__(self, serial=None, low_res=False):
        self.pipeline = rs.pipeline()
...
if __name__ == "__main__":
    # live_application()
    ref_image_path_dict = {
        "135122079702": "/home/jinyang/workspace/20250729-145004.png",
        "242322072982": "/home/jinyang/workspace/20250729-144959.png"
    }
    capture_dict = {
        c: CameraD400(serial=c, low_res=True) for c in ref_image_path_dict.keys()
    }
    # ref_image_path = None
    while True:
        for serial, capture in capture_dict.items():
            ref_image_path = ref_image_path_dict.get(serial, None)
            color_image, depth_image = capture.get_data()
            if ref_image_path is not None:
                ref_image = cv2.imread(ref_image_path)
                color_image = cv2.addWeighted(color_image, 0.5, ref_image, 0.5, 0)
            cv2.imshow(serial, color_image)
        cv2.waitKey(1)