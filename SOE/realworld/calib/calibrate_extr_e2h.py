...
class calibration():
    ...
    def cal(self, calib_wrist):
        ...
        camera_pose = np.load('calib/out/cali_hand/camT.npy')
...
if __name__ == "__main__":
    ...
    intr = np.load('calib/out/cali_hand/intrinsic.npy')
    pattern_size = (11, 8)
    ...
    flangeposList.sort()
    imgwristList = glob.glob(f"calib/out/{dirName}/hand/*c.png")
    imgwristList.sort()
    
    imgList = glob.glob(f"calib/out/{dirName}/side/*c.png")
    imgList.sort()
    ...
    print("="*10)
    print(camT)
    np.save(os.path.join(f'calib/out/{dirName}/','intrinsic.npy'), calib.mtx)
    np.save(os.path.join(f'calib/out/{dirName}/','camT.npy'), camT)
    print("saved")
    while True:
        p.stepSimulation()