...
def cem_search_value_guided(
    dp_module: torch.nn.Module,
    world_model: torch.nn.Module,
    value_fn: torch.nn.Module,
    value_mu: np.ndarray,
    value_sigma: np.ndarray,
    obs_dict: Dict[str, torch.Tensor],
    x0: np.ndarray,
    source_episode_id: str,
    failure_mode_id: int,
    ...
    """Same CEM search as `cem_search`, but the fitness signal directing
    the search is the learned value function V (f2s.value.model, trained
    on real episode-outcome labels -- see f2s/value/train.py and
    SOE/README_F2S.md's "Stepping back" section) evaluated at each
    candidate's world-model-predicted final state, instead of raw
    predicted object-to-goal distance.

    Rationale: `cem_search`'s distance-to-goal fitness is exactly the
    signal already diagnosed to have a weak within-state Spearman
    correlation with real outcome (0.09-0.30 -- see "Guided candidate
    search (CEM)" in the README) -- CEM climbs it hard and finds 0/1278
    real successes, worse than doing nothing. V is trained on a
    different, outcome-supervised signal that a priori might not share
    that specific weakness. This function isolates that one variable
    (what CEM's fitness measures) while keeping the search mechanism
    itself (iterative mean/std refinement over Delta z) identical, so a
    real success-rate difference against `cem_search` and against plain
    random `generate_candidates` is attributable to the fitness signal,
    not to any other change.
    """
    torch.manual_seed(seed)
    z_f = get_latent(dp_module, obs_dict)  # (1, D)
    D = z_f.shape[-1]
    ...
    goal_pos_t = torch.from_numpy(DEFAULT_GOAL_POS.astype(np.float32)).to(device_t)
    obj_slice = P_OBJ_SLICE
    value_mu_t = torch.from_numpy(np.asarray(value_mu)).float().to(device_t)
    value_sigma_t = torch.from_numpy(np.asarray(value_sigma)).float().to(device_t)
    ...
    for it in range(n_iters):
        ...
        with torch.no_grad():
            pred_states = rollout_world_model(world_model, x0_batch, a_batch, horizon=h)  # (h, K, state_dim)
            final_state_norm = (pred_states[-1] - value_mu_t) / value_sigma_t
            v_score = value_fn.predict_proba(final_state_norm)  # (K,), higher is better

        final_obj_pred = pred_states[-1, :, obj_slice]  # (K, 3)
        dist_to_goal = torch.linalg.norm(final_obj_pred - goal_pos_t.unsqueeze(0), dim=-1)  # (K,), for logging only

        pred_states_np = pred_states.permute(1, 0, 2).cpu().numpy()  # (K, h, state_dim)
        action_chunks_np = action_chunks.detach().cpu().numpy()
        dist_np = dist_to_goal.cpu().numpy()
        v_np = v_score.cpu().numpy()
        ...
        # fitness: lower is better, so minimize (1 - V) plus the same
        # soft risk penalty cem_search uses.
        fitness = (1.0 - v_np) + risk_weight * risk_np
        ...
        for k in range(population_size):
            valid = bool(np.isfinite(action_chunks_np[k]).all())
            all_candidates.append(dict(
                candidate_id=f"{source_episode_id}_cemv{it}_{k:03d}",
                source_episode_id=source_episode_id,
                failure_mode_id=failure_mode_id,
                kind="cem_value_guided",
                cem_iteration=it,
                latent_delta=deltas[k].cpu().numpy(),
                action_chunk=action_chunks_np[k],
                valid=valid,
                ...
                predicted_risk=float(risk_np[k]),
                predicted_success=float(success_np[k]),
                predicted_value=float(v_np[k]),
                score=float(success_np[k] - risk_np[k]),
                cem_fitness=float(fitness[k]),
            ))
